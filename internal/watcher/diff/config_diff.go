...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if oldCfg.RemoteManagement.PanelLocalOnly != newCfg.RemoteManagement.PanelLocalOnly {
		changes = append(changes, fmt.Sprintf("remote-management.panel-local-only: %t -> %t", oldCfg.RemoteManagement.PanelLocalOnly, newCfg.RemoteManagement.PanelLocalOnly))
	}
