...
func (w *Watcher) handleEvent(event fsnotify.Event) {
	...
	isIgnoredAuthPath := util.IsIgnoredAuthPath(event.Name, w.authDir)
	isAuthJSON := !isIgnoredAuthPath && strings.HasPrefix(normalizedName, normalizedAuthDir) && strings.HasSuffix(normalizedName, ".json") && event.Op&authOps != 0
